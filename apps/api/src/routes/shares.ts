...
const RESOLVERS: Record<string, (id: string) => Promise<ResourceInfo | null>> = {
  ...
  'procurement-activite': (id) => prisma.activite.findUnique({ where: { id }, select: { ownerId: true, nom: true } }).then((a) => a ? { ownerId: a.ownerId, name: a.nom } : null),
  'procurement-demande-achat': (id) => prisma.demandeAchatWorkflow.findUnique({ where: { id }, select: { ownerId: true, demandeAchatExternalId: true } }).then((d) => d ? { ownerId: d.ownerId, name: `Demande ${d.demandeAchatExternalId}` } : null),
}

const MODULE_LABEL: Record<string, string> = { scrum: 'Scrum Poker', daily: 'Daily', team: 'Équipe', wheel: 'La Roue', capacity: 'Capacité', meetops: 'MeetOps', quiz: 'Quiz', roadmap: 'Roadmap', portfolio: 'Portefeuille', parcourTemplate: 'Template Parcours', parcourInstance: 'Instance Parcours', form: 'Formulaire', pdf: 'PDF Manager', signdoc: 'SignDoc', challenge: 'Challenge innovation', todolist: 'Liste de tâches', tododashboard: 'Tableau de bord To-Do', pi: 'PI Planning', 'procurement-activite': 'Activité (Commande publique)', 'procurement-demande-achat': "Demande d'achat" }
const MODULE_LINK: Record<string, string> = { scrum: '/scrum', daily: '/daily', team: '/equipes', wheel: '/wheel', capacity: '/capacity', meetops: '/meetops', quiz: '/quiz', roadmap: '/roadmap', portfolio: '/portfolio', parcourTemplate: '/parcours/templates', parcourInstance: '/parcours/run', form: '/forms', pdf: '/pdf', signdoc: '/signdoc', challenge: '/innovation/challenges', todolist: '/todo', tododashboard: '/todo/dashboards', pi: '/pi', 'procurement-activite': '/procurement/activites', 'procurement-demande-achat': '/procurement/demandes-achat' }
