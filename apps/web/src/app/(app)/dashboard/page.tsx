...
export default function DashboardPage() {
  const { boards, isLoading, error, boardPresence, createBoard, deleteBoard } = useBoards()
  ...
  const myBoards = filtered.filter((b) => b.role === 'OWNER')
  const sharedBoards = filtered.filter((b) => b.role !== 'OWNER')

  function BoardGrid({ items, showNewCard }: { items: typeof boards; showNewCard?: boolean }) {
    return (
