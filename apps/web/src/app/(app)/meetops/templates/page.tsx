...
export default function MeetopsTemplatesPage() {
  ...
  const filtered = query.trim()
    ? templates.filter((t) =>
        ...
        EVENT_TYPE_LABELS[t.type].toLowerCase().includes(query.toLowerCase())
      )
    : templates
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {templates.length > 0 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 dark:text-white"
          />
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-gray-400">Aucun modèle ne correspond à &quot;{query}&quot;.</p>
      ) : (
