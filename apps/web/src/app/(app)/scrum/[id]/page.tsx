...
function VoteCard({ value, name }: { value: string; name: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="w-14 h-20 rounded-xl bg-white border-2 border-indigo-300 shadow-md flex items-center justify-center">
        <span className="text-xl font-bold text-indigo-700 leading-tight text-center px-1">{value}</span>
      </div>
...
function HiddenCard() {
  return (
    <div className="w-14 h-20 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 shadow-md flex items-center justify-center">
    ...
    <div className="flex flex-col h-full gap-0">
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg bg-indigo-50 border border-indigo-200 px-3 py-1.5 text-sm font-mono font-bold text-indigo-700 hover:bg-indigo-100 transition-colors"
        >
          {room.code}
        ...
        <div className="w-80 shrink-0 flex flex-col gap-3">
          ...
          <form onSubmit={handleAddTicket} className="flex gap-2">
            <input
              ...
              className="flex-1 min-w-0 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
            />
            <button
              type="submit"
              disabled={!newTicketTitle.trim()}
              title="Ajouter"
              className="rounded-xl bg-indigo-600 px-3 py-2 text-white hover:bg-indigo-700 disabled:opacity-40 transition-colors"
            >
            ...
            <button
              ...
              className={`rounded-xl border px-3 py-2 text-sm transition-colors ${showImport ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Bulk import panel */}
          {showImport && (
            <form onSubmit={handleImport} className="flex flex-col gap-2 bg-indigo-50 border border-indigo-200 rounded-xl p-3">
              <p className="text-xs font-semibold text-indigo-700">Importer plusieurs tickets</p>
              <textarea
                autoFocus
                value={importText}
                ...
                rows={5}
                className="w-full text-sm border border-indigo-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none bg-white"
              />
              <div className="flex gap-2 justify-end">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-semibold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-40"
                >
                ...
                <div className="flex items-start gap-2 mb-2">
                  ...
                  {(ticket.estimate || ticket.estimateTime) && (
                    <div className="shrink-0 flex items-center gap-1">
                      {ticket.estimate && (
                        <span className="text-xs font-bold bg-indigo-100 text-indigo-700 rounded-lg px-2 py-0.5 whitespace-nowrap">
                          {ticket.estimate} pts
                        </span>
                      )}
                      {ticket.estimate && ticket.estimateTime && (
          ...
          {!activeTicket ? (
            ...
            <>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <p className="text-xs font-medium text-indigo-500 uppercase tracking-wide mb-1">Ticket en cours</p>
                      ...
                      <div className="border-t border-gray-100 pt-4 shrink-0">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          ...
                          <div className={`col-span-2 flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-colors ${
                            hostVote ? 'bg-green-50 text-green-700' : hostParticipating === false ? 'bg-gray-50 text-gray-400' : 'bg-indigo-50 text-indigo-700'
                          }`}>
                            ...
                            {hostParticipating === null && (
                              <div className="ml-auto flex gap-1.5 shrink-0">
                                <button onClick={() => setHostParticipating(true)} className="px-2 py-0.5 text-xs font-semibold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                                  Je vote
                                </button>
                            ...
                            {hostParticipating === true && !hostVote && <span className="ml-auto text-xs text-indigo-400">En attente…</span>}
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {scaleInfo.values.map((v) => (
                                <button
                                  ...
                                  className="min-w-[36px] px-2 h-10 rounded-xl border-2 border-gray-200 bg-white text-sm font-bold text-gray-700 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all"
                                >
                                  {v}
                                </button>
                              ))}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex gap-4 mb-4">
                        ...
                        {median && <div className="bg-purple-50 rounded-xl px-4 py-2 text-center"><p className="text-xs text-purple-500">Médiane</p><p className="text-xl font-bold text-purple-700">{median}{scaleInfo.suffix ? ` ${scaleInfo.suffix}` : ''}</p></div>}
                      </div>
                    )
                  })()}
            ...
            <div className="flex flex-wrap gap-2">
              {scaleInfo.values.map((v) => (
                <button
                  ...
                  className="min-w-[44px] px-2 h-14 rounded-xl border-2 border-gray-200 bg-white text-sm font-bold text-gray-700 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all"
                >
                  {v}
                </button>
              ))}
